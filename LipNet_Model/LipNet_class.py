...
class LipNet(object):
    ...
    def create(self):
        ...
        if K.image_data_format() == 'channels_first':
            self.input_shape = (self.img_c, self.frame_n,
                                self.img_h, self.img_w)
        else:
            self.input_shape = (self.frame_n, self.img_h,
        ...
        # Stage 2
        # set 1
        self.X = Bidirectional(GRU(256, return_sequences=True,
                                   kernel_initializer='Orthogonal', name='R1'), merge_mode='concat')(self.X)

        # Stage 2
        # set 2
        self.X = Bidirectional(GRU(256, return_sequences=True,
                                   kernel_initializer='Orthogonal', name='R2'), merge_mode='concat')(self.X)

        # Stage 3
        # convert to a fully connected layer
        self.X = Dense(self.output_siz,
                       kernel_initializer='he_normal', name='D1')(self.X)

        # Applying softmax
        self.y_pred = Activation('softmax', name='SM1')(self.X)

        # Model
        self.model = Model(inputs=self.input_data,
                           outputs=self.y_pred, name='LipNet')
